...
public class GeodeFeature extends Feature<NoFeatureConfig> {
	...
	@Override
	public boolean apply(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, NoFeatureConfig config) {
			  ...
			  for(double Y = -radius; Y < radius; Y++ ) {
				  for(double Z = -radius; Z < radius; Z++ ) {
			          if(Math.sqrt((X * X) + (Y * Y) + (Z * Z)) <= radius) {
			            setBlock(reader, new BlockPos(pos.getX() + X,pos.getY() + Y,pos.getZ() + Z), STONE);
			          }
			      }
			  }
		}
